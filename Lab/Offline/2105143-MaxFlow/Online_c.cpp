...
void printMatches(const vector<vector<int>>& residualGraph, int source, int sink) {
    int numOfvertices = residualGraph.size();
    for (int u = source + 1; u <= n; u++) {
        for (int v = n + 1 ; v < sink; v++) {
            if (residualGraph[v][u] == 1) {
                cout << "Man -" << u << " Woman -" << v - n << "\n";
            }
        }
    }
}
int fordFulkerson(const vector<vector<int>>& graph,int source,int sink){
    ...
    cout<<"\n";
    printMatches(residualgraph,source,sink);
    return maxflow;
}
int main(){
    cin >> n >> m;
        ...
        for(int j =0;j<size;j++){
            ...
            if(i>0 && i<=n){
                ...
                if(j>n && j<size-1){
                    ...
                    if(abs(heightOfFemale-heightOfMale)<=10 && abs(heightOfMale-heightOfFemale)<=10 && abs(ageOfFemale-ageOfMale)<=5 && abs(ageOfMale-ageOfFemale)<=5)
                    graph[i][j] = 1;
                }
            }
            if(i>n && i<size-1){
